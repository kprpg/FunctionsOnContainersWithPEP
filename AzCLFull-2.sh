...
export DEMO_ACR=$APP_PREFIX"AzAcrPriv" ## Name must be unique. So please replace "FuncDockerAcr"
...
## Create the Function PLAN with premium SKU based on Linux
...
az functionapp create --name $DEMO_FUNC_NAME --storage-account $DEMO_FUNC_STORAGE_ACCT --resource-group $APP_PE_DEMO_RG \
     ...
     --docker-registry-server-password $ACR_AdminPassword --docker-registry-server-user $ACR_Adminuser
